...
To meet this need, the \pkg{vazul}\footnote{Vazul was an 11th-century
  ...
  blinded data that are unfit for p-hacking.} package
\citep{Nagy2026-gb} in \proglang{R} \citep{R2025} offers flexible and
...
entirely. The \pkg{vazul} package supports this use case directly by
...
The \pkg{vazul} package is open source and distributed via CRAN and
GitHub (\url{https://github.com/nthun/vazul}). All analyses reported
here were conducted using version 1.0.0.
...
The \pkg{vazul} package includes two research datasets for demonstration
and practice. The \texttt{marp} dataset contains cross-national survey
data on religiosity \citep{MARP2022discussion}, while the
\texttt{williams} dataset contains experimental data from a stereotyping
study \citep{Sarafoglou2025-fr}.
